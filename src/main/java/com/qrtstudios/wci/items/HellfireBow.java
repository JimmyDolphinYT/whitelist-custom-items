        ...
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (!itemStack.isEmpty() || bl) {
                if (itemStack.isEmpty()) {
                    itemStack = new ItemStack(WhitelistCustomItems.HELLFIRE_ARROW);
                }

                int i = this.getMaxUseTime(stack) - remainingUseTicks;
                float f = getPullProgress(i);
                if (!((double)f < 0.1)) {
                    boolean bl2 = bl && itemStack.isOf(WhitelistCustomItems.HELLFIRE_ARROW);
                    if (!world.isClient) {
                        HellfireArrowItem arrowItem = (HellfireArrowItem)(itemStack.getItem() instanceof HellfireArrowItem ? itemStack.getItem() : WhitelistCustomItems.HELLFIRE_ARROW);
